        ...
        var defaults = $.extend({
            ...
            $("[data-lightbox-trigger]").on("click", function (e) {
                ...
                if ($("#lightbox").length > 0) {
                    ...
                    var lightbox =
                        '<div id="overlayLightbox"></div>'+
                        '<div id="lightbox">' +
                            '<img id="gifLightbox" src="./imgLightbox/loading.gif"/>' +
                            '<div id="containerLightbox">' +
                                '<div id="closeLightbox"></div>'+
                                '<div id="textLightbox">' +
                                    '<h1 id="titleLightbox">' + titleLightbox + '</h1>'+
                                    '<div id="borderLightbox"></div>'+
                                    '<p id="descriptionLightbox">' + descriptionLightbox + '</p>'+
                                '</div>'+
                                '<div id="contentLightbox">' + //INSERT CLICKED LINK
                                    '<img src="' + imageHref + '" />'+
                                '</div>'+
                                '<div id="arrowsLightbox">'+
                                    '<div id="prevLightbox"></div>'+
                                    '<div id="nextLightbox"></div>'+
                                '</div>'+
                            '</div>'+
                        '</div>';
            ...
            var move = function (num, move, e) {
                    ...
                    $("#contentLightbox, #textLightbox").clearQueue().stop().animate({
                        ...
                        queue: false,
                        duration: 400,
                    }).promise().then(function () {     //WHEN ANIMATION IS ENDED
                        $("#gifLightbox").stop().animate({
                            opacity: 1
                        }, 200)
                        ...
                        $("#lightbox #contentLightbox img").on("load", function () { //WHEN IMAGE IS LOADED  
                            setValues();

                            $("#gifLightbox").stop().animate({
                                opacity: 0
                            }, 100, function () {
                                $("#contentLightbox, #textLightbox").css("left", -move + "px").clearQueue().stop().animate({
                                    left: 0,
                                    opacity: 1
                                }, {
                                    queue: false,
            ...
            $("body").on("click", "#lightbox, #containerLightbox, #closeLightbox", function (e) {
                ...
                timeout = setTimeout(function () { //AFTER ANIMATION REMOVE HTML
